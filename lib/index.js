...
/**
 * 官方路由安全闸：默认拒绝在官方 API 路由上发送截图，避免静默消耗官方额度。
 * 设置环境变量 DSH_VISION_ALLOW_OFFICIAL=1 可放行。
 */
function assertVisionRouteAllowed(exec) {
  if (process.env.DSH_VISION_ALLOW_OFFICIAL === '1') return;
  let provider = '';
  let model = '';
  try {
  ...
  if (/official/i.test(provider)) {
    throw new Error(
      `view 工具默认拒绝在官方 API 路由上发送截图（当前 provider=${provider} model=${model}）。` +
      `原因：截图会作为图片进入模型请求并消耗该路由额度。` +
      `如确需使用，请设置环境变量 DSH_VISION_ALLOW_OFFICIAL=1，或把会话模型切到你自己的订阅/自建路由。`,
    );
  }
}
...
export function apply(ctx) {
  ...
  const disposeComputerMove = ctx.tools.register(makeTextTool(
    'computer_move',
    'Move the Windows mouse cursor smoothly to (x, y). Steps and delay control speed/visibility. Shows the on-screen cursor trail. Auto-starts the overlay (blue frame + Esc abort) unless ensure_overlay=false.',
    {
      type: 'object', additionalProperties: false,
      properties: {
        x: { type: 'integer', description: 'Target X in screen pixels.' },
        y: { type: 'integer', description: 'Target Y in screen pixels.' },
        steps: { type: 'integer', description: 'Interpolation steps (default 30, higher = slower/smoother).' },
        delay_ms: { type: 'integer', description: 'Delay per step in ms (default 15).' },
        ensure_overlay: { type: 'boolean', description: 'Auto-start the overlay (default true).' },
      },
      required: ['x', 'y'],
    },
    async (args) => {
      await ensureOverlay(args.ensure_overlay !== false)
      const out = await runCu('move', ['-X', S(Math.trunc(args.x)), '-Y', S(Math.trunc(args.y)), '-Steps', S(args.steps ?? 30), '-DelayMs', S(args.delay_ms ?? 15)])
  ...
  const disposeComputerClick = ctx.tools.register(makeTextTool(
    'computer_click',
    'Click the Windows mouse at an optional (x, y) — moves there first with a visible glide and a click ripple effect. button: left|right|double. Auto-starts the overlay unless ensure_overlay=false.',
    {
      type: 'object', additionalProperties: false,
      properties: {
    ...
    async (args) => {
      ...
      const extra = []
      if (Number.isFinite(args.x) && Number.isFinite(args.y)) extra.push('-X', S(Math.trunc(args.x)), '-Y', S(Math.trunc(args.y)), '-Steps', '28', '-DelayMs', '14')
  ...
  const disposeComputerTypeReal = ctx.tools.register(makeTextTool(
    ...
    async (args) => {
      ...
      if (action === 'typehuman') extra.push('-DelayMs', '90', '-JitterMs', '35')
      const out = await runCu(action, extra)
